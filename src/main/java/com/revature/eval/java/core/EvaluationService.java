...
public class EvaluationService {
	 ...
	 *
	 * @param phrase Phrase to be made into an acronym
	 * @return Acronym of the phrase
	 */
	 ...
	 * different lengths.
	 *
	 * DEV NOTES - Lengths will be rounded to 3 decimals places
	 */
	public static class Triangle {
		...
		public boolean isEquilateral() {
			int s1 = (int) (Math.round(this.sideOne * 1000));
			int s2 = (int) (Math.round(this.sideTwo * 1000));
			int s3 = (int) (Math.round(this.sideThree * 1000));
			return s1 == s2 && s2 == s3;
		}
	...
	public int getScrabbleScore(String string) {
		...
		for (int i = 0; i < string.length();i++) {
			switch (string.charAt(i)) {
				case 'A': case 'E': case 'I': case 'O': case 'U': case 'L': case 'N': case 'R': case 'S': case 'T': score++;
					break;
				...
				default:
					System.out.println("Unrecognized character: " + string.charAt(i));
			}
		}
		return score;
	}
	...
	public boolean isArmstrongNumber(int input) {
		...
		for (int i = 0; i < in.length();i++) {
			result += Math.pow(Character.getNumericValue(in.charAt(i)), in.length());
		}
		return input == result;
	}
	...
	public static class RotationalCipher {
		...
		public String rotate(String string) {
			...
			String result = "";
			for (int i = 0; i < string.length(); i++) {
				ascii = string.toLowerCase().charAt(i);
				if (ascii > 96 && ascii < 123 ) {
					result += (char) ((((ascii % 97) + key) % 26) + 97);
				} else {
					result += (char) (ascii);
				}
			}
	 ...
	 *
	 * @param i Index of the prime number, e.g. 6 for 6th prime number.
	 * @return The prime number at that index.
	 */
	public int calculateNthPrime(int i) {
		List<Integer> primes = new LinkedList<Integer>();
		primes.add(2);
		int index = 2;      //Index for found primes, e.g., 1st, 2nd, 3rd,...
		int counter = 2;    //Counts up to see if the next integer is prime.
		if (i == 1) return 2;
		boolean isFound = false;
		/**
		 * Notes: The most common way is to build a list of primes by moving through each integer and
